...
public partial class AccessRule : IEntity<AccessRuleModel>
{
    ...
    /// <param name="actionKind">动作。放行/拦截/限流</param>
    /// <param name="limitDimension">限流维度。IP/用户</param>
    ...
    public static IList<AccessRule> Search(AccessActionKinds actionKind, LimitDimensions limitDimension, Boolean? enable, DateTime start, DateTime end, String key, PageParameter page)
    {
        var exp = new WhereExpression();

        if (actionKind >= 0) exp &= _.ActionKind == actionKind;
        if (limitDimension >= 0) exp &= _.LimitDimension == limitDimension;
