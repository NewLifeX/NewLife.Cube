...
namespace NewLife.CubeUI.Helpers
{
    /// <summary>
    /// HttpClient助手
    /// </summary>
    public static class HttpClientHelper
    {
        ...
        /// <param name="dataName">数据字段名称，默认data。同一套rpc体系不同接口的code/message一致，但data可能不同</param>
        /// <returns></returns>
        public static TResult ProcessResponse<TResult>(HttpResponseMessage response, String dataName = "data")
        {
            var rtype = typeof(TResult);
            if (rtype == typeof(HttpResponseMessage)) return (TResult)(Object)response;

            var buf = response.Content == null ? null : response.Content.ReadAsByteArrayAsync().Result;
