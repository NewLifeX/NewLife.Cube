/**
 * Mermaid 图表渲染工具（CDN 懒加载）— 移植自 Vue 皮肤 ai/mermaidRenderer.ts
 *
 * - 规范化修复 LLM 生成的 Mermaid 代码
 * - 首次遇到图表才从 CDN 加载 mermaid 并初始化一次（不打包 2.6MB）
 * - parse 校验 + render 渲染 SVG，失败回退源码展示
 */
...
/** 修复 Markdown 表格管道 `|` 与 Mermaid 连线 `-->` 混淆 */
...
/** 移除 classDef 中 Mermaid 不支持的 SVG 几何属性 */
...
/** 修复 gantt 图的纯时间 dateFormat，补虚拟日期 */
...
/** XSS 防护：剥离 script/iframe 等标签与 on* 事件属性 */
...
/** 解析可渲染的 Mermaid 代码：先原始、后规范化，用 parse 校验 */
...
export function renderMermaidBlocks(container: HTMLElement | null | undefined, mermaidUrl: string): void {
  ...
  void getMermaid(mermaidUrl)
    .then((mermaid) => {
      codes.forEach((el) => renderMermaidBlock(el, mermaid));
    })
    .catch(() => {
      // CDN 加载失败：保持源码块
    });
