...
# Vitest 单元/组件测试
pnpm test:unit      # vitest run --config vitest.config.unit.ts

# Playwright 组件视觉测试（CT）— 见 docs/guides/component-visual-dev.md
pnpm ct:server      # 启动 gallery（端口 5190），手动浏览器预览组件
pnpm test:ct        # 无头模式，截图对比基线
pnpm test:ct:headed # 有头模式，肉眼观察渲染
pnpm test:ct:update # 更新基线截图（确认修改后）
pnpm test:ct:dev    # 开发循环：有头 + 截图存档（不对比）
