<template>
  ...
  >
    <draggable
      v-model="fileList"
      :item-key="(val: UploadUserFile) => getItemSrc(val)"
      class="flex flex-wrap"
      :animation="200"
    >
      <template #item="{ element, index }">
        <div
          class="relative mr-2 flex group"
          @click.stop
          :key="getItemSrc(element)"
          v-if="fileList.length <= limit || index >= limit"
        >
          <el-image
            fit="cover"
          ...
          >
            <template #error>
              <div
                class="bg-gray-200 w-full h-full flex justify-center items-center font-bold text-green-500 text-xl flex-col border border-green-500"
          ...
          <div
            ...
            v-if="element.status === 'ready' || element.status === 'uploading'"
          >
            <el-progress
              :text-inside="true"
              :stroke-width="20"
          ...
          <div
            class="!absolute right-0 top-0 bg-black/80 w-6 h-6 flex justify-center items-center rounded-tr opacity-0 group-hover:opacity-100 duration-75"
      ...
      <template #footer>
        <div
          class="avatar-uploader w-28 h-28 flex justify-center items-center flex-col"
          key="btn"
          :class="{ 'opacity-50': limit !== 1 && fileList.length >= limit }"
        >
          ...
          <template v-else>
            ...
            <div class="text-gray-500 text-sm mt-2" v-if="limit > 1">
...
const cfg = getConfig();
const action = cfg.request.baseUrl + props.url;
...
const getItemSrc = (item: UploadUserFile & { rawUrl?: string }) => {
  let url = '';
  if (item.rawUrl) url = item.rawUrl;
  else if (item.raw && item.raw.type.indexOf('image') === 0) {
    item.rawUrl = URL.createObjectURL(item.raw);
    url = item.rawUrl;
  } else if (item?.url) url = baseUrl + item.url;
  else if (item?.response) url = baseUrl + (item.response as any).data[props.resultKey];
  return url;
};
