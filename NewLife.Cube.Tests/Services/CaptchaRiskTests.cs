...
/// 使用内存缓存（TestCacheProvider）构造 VerifyCodeService，不依赖数据库。
/// </summary>
public class CaptchaRiskTests : IDisposable
{
    // 与 AuthCacheKeys 缓存前缀常量保持一致（内部实现耦合，注释说明；变更 AuthCacheKeys 常量时需同步）
    ...
    private readonly VerifyCodeService _verifyCode;
    ...
    {
        Assert.Equal(0, _verifyCode.GetRiskLevel(ip, null));
    }
    ...
    {
        Assert.Equal(1, _verifyCode.GetRiskLevel(ip, null));
    }
    ...
    public void GetRiskLevel_PublicIpWithFail_ReturnsTwo()
    {
        ...
        Assert.Equal(2, _verifyCode.GetRiskLevel(ip, null));
    }
    ...
    public void GetRiskLevel_UserWithFail_RiskUp()
    {
        ...
        Assert.Equal(2, _verifyCode.GetRiskLevel("8.8.8.8", user));
    }
    ...
    public void GetRiskLevel_Blocked_ReturnsThree()
    {
        ...
        Assert.Equal(3, _verifyCode.GetRiskLevel(ip, null));
    }
    ...
    public void RequireCaptcha_SceneForced_AlwaysTrue()
    {
        ...
        Assert.True(_verifyCode.RequireCaptcha(1, "127.0.0.1"));
        Assert.True(_verifyCode.RequireCaptcha(1, "8.8.8.8"));
    }
    ...
    public void RequireCaptcha_RiskOff_NoCaptcha()
    {
        ...
        Assert.False(_verifyCode.RequireCaptcha(1, "8.8.8.8"));
    }
    ...
    public void RequireCaptcha_InnerIp_NoCaptcha()
    {
        ...
        Assert.False(_verifyCode.RequireCaptcha(1, "127.0.0.1"));
        Assert.False(_verifyCode.RequireCaptcha(2, "192.168.1.5"));
    }
    ...
    public void RequireCaptcha_PublicIpWithFail_RequireCaptcha()
    {
        ...
        // 公网+失败 = 风险2 >= 阈值2 → 要求
        Assert.True(_verifyCode.RequireCaptcha(1, ip));
    }
    ...
    public void RequireCaptcha_TrustedDevice_Exempt()
    {
        ...
        // 未标记前要求
        Assert.True(_verifyCode.RequireCaptcha(1, ip, null, deviceId));

        // 标记可信后豁免
        _userService.SetTrustedDevice(deviceId, ip);
        Assert.False(_verifyCode.RequireCaptcha(1, ip, null, deviceId));
    }
