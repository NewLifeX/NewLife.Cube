/** 页面仪表盘 Widget 协议（OSC-2608280e9e）。前后端共用形状。 */
...
export function parseDashboardJson(json: string | null | undefined): DashboardConfig | null {
...
function normalizeWidget(raw: unknown, index: number): WidgetInstance | null {
  ...
  const layout: WidgetLayout = { w: w as 3 | 4 | 6 | 12, order };
...
export function serializeDashboardJson(cfg: DashboardConfig): string {
  const widgets = [...(cfg.widgets ?? [])]
    ...
    .map((w, i) => {
      ...
      return {
        ...
        layout: {
          ...w.layout,
          w: (WIDTHS.has(lw) ? lw : 3) as 3 | 4 | 6 | 12,
          order: i,
        },
      };
    });
...
export function validateDashboardForPut(
  ...
  if (parsed.widgets.length > MAX_WIDGETS) return { ok: false, error: '部件数量不能超过 12' };
  ...
  return { ok: true, json: serializeDashboardJson(parsed) };
}
