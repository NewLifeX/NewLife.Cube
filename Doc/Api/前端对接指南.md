...
### ArcoVue：EntityViewProfile 与命名视图（OSC-0005）
...
- OSC-0005 仅创建/使用 `view=table`；默认种子名称「默认列表」（读取时可将旧「列表」迁移为「默认列表」）。树/卡片/甘特类型切换见后续 OSC。
- 列表主舞台为 VisActor VTable；排序参数 `sort` + `desc`。
- 打开详情：数据行**双击**或操作列「详情」→ 右侧抽屉（单击不打开，避免误触）。
