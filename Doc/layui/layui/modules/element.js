...
layui.define('jquery', function(exports) {
    ...
    , call = {
        ...
        tabClickCube: function(e, index, liElem, options) {
            options = options || {};
            var othis = liElem || $(this),
                index = index || othis.children('li').index(othis),
                parents = $(options.headerElem),
                item = $(options.bodyElem).children('.layui-tab-item'),
                elemA = othis.find('a'),
                isJump = elemA.attr('href') !== 'javascript:;' && elemA.attr('target') === '_blank', //是否存在跳转
                unselect = typeof othis.attr('lay-unselect') === 'string', //是否禁用选中
                filter = parents.attr('lay-filter');
        ...
        tabDeleteCube: function(e, othis) {
            var li = othis || $(this).parent(),
                index = li.index(),
                parents = li.parents('.cube-tabs').eq(0),
                item = $('.cube-tabs-content').children('.layui-tab-item'),
                filter = parents.attr('lay-filter');


            var opt = {};
            opt.headerElem = '.cube-tabs';
            opt.bodyElem = '.cube-tabs-content';

            if (li.hasClass(THIS)) {
                if (li.next()[0]) {
                    call.tabClickCube.call(li.next()[0], null, index + 1, opt);
                } else if (li.prev()[0]) {
                    call.tabClickCube.call(li.prev()[0], null, index - 1, opt);
                }
            }
