...
- **AI 对话助手**：`AiChat` Action（薄壳）+ `IAIChatService` 服务下沉（会话管理/工具循环/SSE 事件）。工具集含 `get_data_context`（数据上下文）/`get_form_schema`（编辑模式带已有值）/`fill_form`（智能填表）/`get_page_context`（页面上下文，非实体页可用：`IPageDataContext` 服务端优先/浏览器采集兜底）/`get_system_info`（系统诊断）/`run_js`（页面脚本执行，SSE 下发 + 前端回传，检查点经事件总线广播支持分布式集群），均带中文描述供 LLM 决策。非实体页面可继承 `AiPageControllerBase` 获得 AiChat 端点。SSE 事件驱动前端填表与工具卡片展示，MVC 与 Vue 双端浮窗。
