...
- id: OSC-0015
- state: Done
- updated: 2026-08-06T22:30:00+08:00
...
- 追加工单 `3874b117`（分组 VTable 原生 groupBy 重构）与 `efb3bec4`（徽标主题色 / 移除排序按钮 / 树视图配置无分组）在 `tasks.md` 新增 T6 登记，`verify.md` 对应补充 AC-11/12/13；详见 tasks.md T6 与 verify.md。

## 验收记录（2026-08-06，见 verify.md 完整明细）

- 全量 Vitest：26 文件 / 266 用例全部通过；vue-tsc + vite 构建通过（`npm run build` EXIT=0）。
- 三步编排：implementation-audit / code-review / doc-sync 完成，摘要见 verify.md。
