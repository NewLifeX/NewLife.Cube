...
| 3 | 筛选操作符**按字段类别开放**（纯前端匹配，不依赖后端算子）：枚举/值集（等于/不等于/为空/不为空）、字符（等于/不等于/包含/不包含/为空/不为空）、人员（等于/不等于，用户下拉）、数字（等于/不等于/大于/大于或等于/小于/小于或等于/为空/不为空，不含范围）、日期时间（等于/晚于/早于/为空/不为空）。 |
...
| 9 | **筛选为纯前端过滤**：条件不并入后端请求，对已加载数据本地过滤，翻页继续过滤；操作符按字段类别开放（枚举/字符/人员/数字/日期时间），不依赖后端算子。 |

## 3. 做什么

- 在 `NamedView` 增加 `filter`（筛选构建器方案）与 `group`（分组字段列表）两个域，序列化/反序列化透传并 round-trip 保留未知字段。
- 实现 `matchesViewFilter`：对已加载数据行按筛选方案匹配（eq/neq/contains/notContains/isNull/notNull/gt/gte/lt/lte/after/before 全操作符）；`filterBuilder` 提供字段类别 → 操作符矩阵与值控件选择。
- 新增 `FilterBuilderPopover`（弹层条件构建：条件行增删、AND/OR 切换、按类别操作符与值控件、人员字段用户下拉、应用/保存到视图/清除）。
...
| ArcoVue Vitest | 是 | ViewFilter 解析/round-trip/非法归一、字段类别/操作符矩阵、matchesViewFilter 全操作符（eq/neq/contains/isNull/gt/gte/after/before 等）、groupRows 多级分组/计数/未知字段/折叠 key、QueryInsightPanel 折叠逻辑 |
