...
| 工期 | T1–T11 主实现 + T12 验收缺口补齐；2026-08-19 删除重复的 AutomationRun 实体表 |
...
- 运行队列为内存 `AutomationRun` POCO；终态与 `GET /Runs` 写/读系统 `Log`（Action=`Automation`）。（2026-08-16 曾落独立运行表，2026-08-19 已删，勿回滚。）
...
3. **验收缺口清单分级后一次性补齐**：T12 把 P0–P2 一次补完。P0.1 当时用独立表对齐旧 design，事后证明与系统 Log 双写；2026-08-19 以 Log 为唯一审计源。
4. **前后端 Filter 同构用单测锁死**：缺字段 isNull、contains 大小写、非数字比较等与 `matchesViewFilter` 对齐。

## 待改进

1. **不要为队列另造实体表当 Runs 源**：终态已有系统 `Log`（Action=`Automation`）就不要再落 `AutomationRun` 表。T12.1 的「必须落库」已废止；后续审计以 design 文首现行约束为准。
...
- 进程重启丢失 in-flight queued/waiting；delay 只在本进程续跑（🟢 可接受；**勿为此再加 `AutomationRun` 实体表**）。
