...
| home | `Admin/Index` 短路 | `DefaultHome`（监控页）。静态路由 `/home` 为 **Workbench** 自定义工作台（OSC-26082815a1），不再与 Index 同用 |
...
- **查询洞察面板（OSC-2608280e9e）**：`InsightPanel` 为洞察槽 `WidgetHost`（`features/widget/`）。部件配置存 `ViewProfile.dashboardJson`（与 viewsJson 分域）。洞察槽平台 kind：指标卡 / 迷你图表（迷你看板仅 `surface=workbench`）；`main.ts` 调用 `registerPlatformWidgets()`，业务可 `registerWidget` 再注册。查询走 `cubeApi.widget.query`，随**筛选构建器**重查，**不**随 SearchDrawer。空配置且可编辑时显示「添加部件」；只读空槽高度 0。旧 `showStat`/`showChart`/`chartOption` 仅在 dashboard 未配置时只读合成。视图可分享（embed + 短令牌）。**规范：InsightPanel 不再提供搜索入口，搜索/预定义查询管理只走 `SearchDrawer`。**
- **首页工作台（OSC-26082815a1）**：`/home` → `views/home/Workbench.vue`；`GET /Cube/Workbench` 用户>主角色>系统种子。named 对等 CubeNC KPI/内容 + Inbox。管理员 `/settings/workbench-role` 写角色模板。`/Admin/Index` 仍系统信息/进程表。
...
- **搜索面板一行折叠（OSC-0015）**：搜索字段容器默认一行，溢出时底部显示「展开更多 N」/「收起」（`offsetHeight > clientHeight` 判定），字段增删后重置折叠态；**已随 OSC-0016 面板重构改为右侧搜索抽屉**（内联搜索区随 `InsightPanel` 搜索区移除一并停用）。
...
- **用户图表配置（OSC-260819e483，OSC-2608280e9e 起降为只读合成）**：未保存 `dashboardJson` 时仍可从旧 `insight.chartOption` / `OnGetChartData` 合成 `legacyChart`；保存仪表盘后 insight 不再作为写入权威。新编图表走迷你图平台模板。
...
- **ListField 自定义链接分流（OSC-2608178bdb）**：GetPage `list[]` 中 `url` 非空时——实体字段挂链接留在**数据列单元格**可点；合成 `AddListField` 导航链接与 `dataAction` 动作**不占空洞列**，并入 `__ops`（直出≤2 +「更多」溢出）；卡片/看板底栏同源；日历/甘特在详情抽屉标题区展示。
