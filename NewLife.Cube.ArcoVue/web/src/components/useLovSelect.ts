...
/** LovSelect 组件 props 类型（与 LovSelect.vue defineProps 泛型逐字一致） */
...
/** LovSelect 组件 emits 类型（与 LovSelect.vue defineEmits 泛型逐字一致） */
interface LovSelectEmits {
  'update:modelValue': [value: string | number | Array<string | number> | undefined];
}

type LovSelectEmit = <K extends keyof LovSelectEmits>(event: K, ...args: LovSelectEmits[K]) => void;

/** LovSelect 组件全部业务 TS：LOV 元数据加载/远程搜索/标签回显（自 LovSelect.vue script setup 原样搬移） */
export function useLovSelect(props: LovSelectProps, emit: LovSelectEmit) {
