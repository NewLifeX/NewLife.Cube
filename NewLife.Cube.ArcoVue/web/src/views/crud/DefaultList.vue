 ...
 * fieldName 由列表/树视图点击携带；卡片/看板未携带时回退到 Enable 字段（兼容）。
 * 先乐观更新本地行——按切换后的实际值即时展示（开→success 徽标、关→danger 徽标，双向而非单一禁用态），
 * 再调后端接口确认；成功后 loadData 权威刷新，失败回滚并提示。Enable 字段走既有 enableSelect/disableSelect，其余走 setField。
 */
async function onToggleEnable(row: Record<string, unknown>, fieldName?: string) {
  ...
  try {
    if (field.name.toLowerCase() === 'enable') {
      ...
      await cubeApi.page.setField(typePath.value, [id as string | number], field.name, target);
