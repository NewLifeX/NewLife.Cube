...
public partial class ReadOnlyEntityController<TEntity> : ControllerBaseX where TEntity : Entity<TEntity>, new()
{
    ...
    public virtual IActionResult ImportZip(IFormFile file)
    {
        ...
        foreach (var entry in zip.Entries)
        {
            ...
            if (entry.Name.EndsWith(".db")) names.Add(entry.Name.TrimEnd(".db"));
        }

        WriteLog("导入Zip", true, $"导入[{names.Join()}] 共{rs}行");
    ...
    {
        if (!name.EndsWith(".db")) return 0;
        name = name.TrimEnd(".db");

        // 从名称定位实体工厂
        var type = Type.GetType(name) ?? name.GetTypeEx();
        ...
        if (factory == null)
        {
            ...
            if (name.EqualIgnoreCase(type.Name, type.FullName)) factory = Factory;
        }
        if (factory != null)
        {
