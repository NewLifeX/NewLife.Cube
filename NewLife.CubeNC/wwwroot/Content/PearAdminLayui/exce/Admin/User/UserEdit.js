layui.use(['xmSelect', 'commonExtend', 'upload'], function () {
    ...
    var user_Enable=xmSelect.render({
        el: '[xid=enable]',
        name: "Enable",
        radio: true,
        clickClose: true,
    ...
    user_Enable.setValue([$("#user_Enable").val()]);
    console.log($("#user_Sex").val());
    $("input:radio[value='" + $("#user_Sex").val() + "']").prop("checked",true);
    ...
    apiUtil.ajax('/Admin/User/GetRole', [], "get", function (res) {
        ...
        user_RoleID.setValue([$("#user_RoleID").val()]);
        var arr = $("#user_RoleIDs").val().split(',');
        user_RoleIDs.setValue(arr);
    });

    //上级部门
    apiUtil.ajax('/Admin/User/GetSelectDeparTree', {}, "get", function (res) {
        var user_DepartmentID = xmSelect.render({
        ...
        user_DepartmentID.setValue([$("#user_DepartmentID").val()]);
    });
    ...
    form.on('submit(saveBtn)', function (data) {
        ...
        apiUtil.ajax('/Admin/User/Edit', data.field, "POST", function (res) {
            if (res.code == 0) {
                apiUtil.swalSuccess({ title: "编辑用户成功" }, function () {
                    parent.refeshTable();
                ...
                $(obj).text("确定保存");
                apiUtil.swalError("编辑用户失败", res.msg);
            }
        });
        return false;
    });
