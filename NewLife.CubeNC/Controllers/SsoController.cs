...
namespace NewLife.Cube.Controllers
{
    ...
    public class SsoController : Controller
    {
        ...
        public virtual ActionResult Login(String name)
        {
            ...
            var state = Request.GetRequestValue("state");
        ...
        public virtual ActionResult LoginInfo(String code, String state)
        {
            ...
            try
            {
                ...
                // 标记登录提供商
                HttpContext.Session.SetString("Cube_Sso", client.Name);
                HttpContext.Session.Set("Cube_Sso_Client", client.ToBytes());
            ...
            catch (Exception ex)
            {
                ...
                var inf = new Exception("",ex);
                //var inf = new HandleError(ex, "Sso", nameof(LoginInfo));
        ...
        {
            // 先读Session，待会会清空
            var client = HttpContext.Session.Get<OAuthClient>("Cube_Sso_Client");
            ...
            if (client != null)
            {
                if (!client.LogoutUrl.IsNullOrEmpty())
                {
                    ...
                    // 跳转到验证中心注销地址
                    url = client.Logout(url, state, Request.GetEncodedUrl().AsUri());
        ...
        public virtual ActionResult Bind(String id)
        {
            ...
            var redirect = prov.GetRedirect(Request, Request.Headers["Referer"].FirstOrDefault() + "");
            // 附加绑定动作
