...
namespace NewLife.Cube.Web
{
    ...
    public class SsoProvider
    {
        ...
        {
            var url = request.GetRequestValue("r");
            if (url.IsNullOrEmpty()) url = request.GetRequestValue("redirect_uri");
            if (url.IsNullOrEmpty() && referr) url = request.Headers["Referer"].FirstOrDefault() + "";
        ...
        {
            if (returnUrl.IsNullOrEmpty()) returnUrl = request.GetRequestValue("r");
            // 过滤环回重定向
            if (!returnUrl.IsNullOrEmpty() && returnUrl.StartsWithIgnoreCase("/Sso/Login")) returnUrl = null;

            var uri = RedirectUrl.AsUri(new Uri(request.GetEncodedUrl())) + "";
        ...
        public virtual String OnLogin(OAuthClient client, IServiceProvider context)
        {
            ...
            if (req != null) forceBind = req.GetRequestValue("sso_action").EqualIgnoreCase("bind");
