...
layui.define(['table', 'form', 'laydate', 'util', 'excel', 'laytpl'], function (exports) {
  ...
  var mod = {
    ...
    , soulReload: function (myTable, isr) {
      ...
      else if (typeof myTable.filterUrl !== 'undefined') {
          //2022.03.31:增加filterUrl后端提交from表单刷新展示
          if (myTable.filterMethod === "from") {
              //location.href = myTable.filterUrl + "?filter=" + encodeURIComponent(JSON.stringify(myTable.where));
              var form = $("<form method='post'></form>");
              var input;
              form.attr({ "action": myTable.filterUrl });
              $.each(myTable.where, function (key, value) {
